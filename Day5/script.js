if (localStorage.getItem("lightTheme") === null) {
  ...
  var lightThemeStatus = true;
} else {
  var lightThemeStatus = JSON.parse(localStorage.getItem("lightTheme"));
}

//previous challenge code from here
...
let onloadTheme = lightThemeStatus ? lightTheme : darkTheme;
for (var i = 0; i < Object.keys(onloadTheme).length; i++) {
  document.documentElement.style.setProperty(
    `--${Object.keys(onloadTheme)[i]}`,
    onloadTheme[Object.keys(onloadTheme)[i]]
  );
}
...
function switchTheme() {
  ...
  localStorage.setItem("lightTheme", JSON.stringify(lightThemeStatus));
